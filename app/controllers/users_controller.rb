class UsersController < ApplicationController
  ...
  load_and_authorize_resource :except => [:current, :update_role, :dali_documents]
    ...
    respond_to do |format|
      format.html{
        if !params[:page] || params[:tab] == "dali_documents" || (params[:page] && (params[:page] == 1))
          render :partial => 'dali_documents/profile_dali_documents_list', :locals => {:scope => :me, :limit => 0, :page=> 1, :sort_by=> params[:sort_by]||"updated_at"}, :layout => false
        else
          render :partial => 'dali_documents/profile_dali_documents_page', :locals => {:scope => :me, :limit => 0, :page=> params[:page], :sort_by=> params[:sort_by]||"updated_at"}, :layout => false
        end
      }
    end
  end
